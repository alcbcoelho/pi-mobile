	...
	const page =
		foundObjects.length || lostObjects.length ? (
			<TabBar
				...
				hasBadge={[RegisteredObjectsData.lostObjects.length, RegisteredObjectsData.foundObjects.length]}
			/>
		) : (
			<Home />
		);
		...
		<>
			...
			<Portal>
				<Snackbar visible={route?.params ? true : false}>
					{RegisteredObjectsData[route?.params?.foundObject ? 'foundObjects' : 'lostObjects'][
						route?.params?.objectId - 1
					]?.object + ' apagado com sucesso'}
				</Snackbar>
			</Portal>
		</>
	);
